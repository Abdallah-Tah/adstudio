...
| 3 (M2) | Video, QC, audio, render | **Built 2026-07-15 — at STOP GATE 3 pending live runs.** Two selectable fal video engines (`VIDEO_ENGINE=kling` default / `ltx`), crv+Haiku QC gate w/ capped auto-retry, ElevenLabs VO + timestamp spans, licensed music library (`music-add`), ffmpeg render (vendored freecut f1d4334), `/produce` + `/status` + `/report`, `qc-eval` CLI. Live exit criteria need: product photos (benchmark), FAL/ANTHROPIC/ELEVENLABS keys, licensed tracks. |
